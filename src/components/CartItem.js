...
const CartItem = (props) => {
  ...
  return (
    <div className="cart-list-item">
      <button
        className="cart-list-item-delete" type="button" onClick={() => deleteItem(props.item.id)}><IoMdTrash/></button>
      ...
      <span className="cart-list-info">{(props.item.price * props.item.stock).toFixed(2)}$</span>
    </div>
  );
};
